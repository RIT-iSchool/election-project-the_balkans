  ...
  return async function(req: Request, _res: Response, next: NextFunction) {
    try {
      ...
      if (new Date() > sessionData.expiresAt) {
        throw new AuthenticationError("Session is expired. Please log in again.");
      }
      ...
      if (typeof societyId !== 'string') {
        throw new AuthenticationError("Invalid society ID. Check your headers.");
      }

      // Make sure the society exists in our database
      const [societyData] = await db.select().from(society).where(eq(society.id, parseInt(societyId)));
      ...
      const [societyMemberData] = await db.select().from(societyMember).where(and(eq(societyMember.userId, userData.id), eq(societyMember.societyId, parseInt(societyId))));
      if (!societyMemberData) throw new Error('User does not have access to this society');
      ...
      if (role && !(roleHasAccess(societyMemberData.role, role) || userData.admin)) {
        throw new AuthenticationError("Role is not able to access this resource.")
      }
